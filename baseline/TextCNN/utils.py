...
@Author: Xiao Tong
@FileName: utils.py
@CreateTime: 2022-04-30 20:02:02
@Description:
...
def load_khan_data_by_id(filepath: str,
    ...
    load single sample file, extract subtitles and finish embedding at the same time.
    """
...
def metric_EMR(predicts: np.ndarray, labels: np.ndarray, num_classes_list, threshold: float=0.5):
    # predict_labels = (predicts >= threshold).astype(np.float32)
    predict_labels = get_predictions(predicts, num_classes_list, threshold)
