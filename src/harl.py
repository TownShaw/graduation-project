...
class TCA(torch.nn.Module):
    ...
    def forward(self, word_x, last_w):
...
class HAM(torch.nn.Module):
    ...
    def forward(self, text_feature, text_avg, w_h_last, local_output_list, local_scores_list):
        r_att, W_att = self.tca(text_feature, w_h_last)
        cpm_input = torch.cat([r_att, text_avg], dim=-1)
        A_L, P_L = self.cpm(cpm_input)
        w_h = self.cdm(P_L, W_att)
        local_output_list.append(A_L)
        local_scores_list.append(P_L)
        return text_feature, text_avg, w_h, local_output_list, local_scores_list
...
class HARL(torch.nn.Module):
    ...
    def forward(self, rnn_out, rnn_avg):
        ...
        local_scores_list = []
        # let w_0 = input_x
        self.net_sequence(rnn_out, rnn_avg, rnn_out, local_output_list, local_scores_list)
